...
The returned ID and revision identify the stored record. Reuse a request key only for an exact retry. A retry returns the record ID and original record revision without another write; hydrate that ID to read current content. For corrections, reclassification or supersession, inspect `guide --data '{"operation":"memory.update"}'`, read the existing record and use the expected memory record revision. An update replaces the canonical record without storing its prior payload. Explain a change through evidence; do not silently turn a hypothesis into a fact or erase an unresolved contradiction.
